...
import { PlusIcon, SendIcon, FileTextIcon, EditIcon, TrashIcon, ClockIcon, HelpCircleIcon, AlertCircleIcon, CheckCircleIcon, InfoIcon, MapPinIcon, XIcon, SaveIcon, AlertTriangleIcon, FileIcon, DatabaseIcon, PenIcon } from 'lucide-react';
...
export const TechnicianDashboard = ({
  ...
  // État pour l'affichage des données exportées
  const [exportedOrder, setExportedOrder] = useState(null);
  // État pour afficher le message de confirmation de génération de données
  ...
  // Générer des données fictives pour l'historique
  const generateMockData = () => {
    generateMockOrders(technicianInfo.name, 15);
  ...
  return <div className="animate-fadeIn">
      ...
      <div className="bg-blue-50 p-4 rounded-lg mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <button onClick={generateMockData} className="flex items-center text-gray-600 hover:text-gray-800 text-sm bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-md transition-colors" title="Générer des données fictives pour l'historique">
      ...
      {/* Message de confirmation pour les données fictives */}
      {showMockDataConfirmation && <div className="bg-green-50 border border-green-200 text-green-700 p-3 rounded-md mb-4 flex items-center animate-fadeIn">
          <CheckCircleIcon className="w-5 h-5 mr-2 text-green-500" />
          <p>15 ordres fictifs ont été générés et ajoutés à l'historique.</p>
        </div>}
                ...
                {orders.map(order => <div key={order.id} className="bg-white border rounded-md p-3 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <button onClick={() => openSignatureModal(order.id)} className="mt-2 flex items-center text-xs text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-md border border-blue-200 shadow-sm hover:shadow transition-all duration-150 font-medium">
                          <PenIcon className="w-3 h-3 mr-1" />
                          {order.signature ? 'Modifier signature' : 'Ajouter signature'}
                        </button>
                      </div>
