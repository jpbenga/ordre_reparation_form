...
export function App() {
  ...
  // État pour le modal d'envoi par email
  const [showEmailModal, setShowEmailModal] = useState(false);
  const [ordersToEmail, setOrdersToEmail] = useState([]);
  ...
  const handleSendEmail = async email => {
    ...
    // Dans une application réelle, vous appelleriez ici votre API pour envoyer l'email
    console.log(`Envoi des ordres à ${email}`, ordersToEmail);
    // Si on envoie tous les ordres actifs
    if (ordersToEmail === repairOrders || ordersToEmail.length === repairOrders.length) {
  ...
  return <div className="flex flex-col min-h-screen w-full bg-white">
      ...
      <EmailSendModal isOpen={showEmailModal} onClose={handleCloseEmailModal} ordersCount={ordersToEmail.length} recipientEmail="facturation@euromaster.com" onSendEmail={handleSendEmail} />
    </div>;
}